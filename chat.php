...
$moodEmoji = '😊';
if ($weekMood !== null) {
    if ($weekMood >= 0.6) $moodEmoji = '😊';
    elseif ($weekMood >= 0.4) $moodEmoji = '😐';
    else $moodEmoji = '😔';
}
?>
<!DOCTYPE html>
...
<body class="antialiased" style="background-color: var(--bg-primary);">
    ...
    <aside id="sidebar-menu" class="sidebar-menu">
        ...
        <div class="sidebar-content">
            ...
            <div class="notifications-section">
                ...
                <button class="menu-btn" onclick="Menu.markAllNotificationsRead()" style="font-size: 0.75rem; color: var(--text-tertiary);">
    ...
    <header class="fixed top-0 left-0 right-0 z-50" style="background-color: var(--bg-secondary); border-bottom: 1px solid var(--border-color);">
        <div class="max-w-2xl mx-auto px-4 py-3 flex justify-between items-center">
            ...
            <div class="flex items-center gap-2">
                <span class="text-sm hidden sm:inline" style="color: var(--text-secondary);"><?= htmlspecialchars($user['name']) ?></span>
                <button onclick="confirmLogout()" class="p-2 transition-colors" style="color: var(--text-tertiary);" title="Cerrar sesión">
        ...
        <div class="max-w-2xl mx-auto">
            ...
            <div id="messagesContainer" class="min-h-[calc(100vh-13rem)] px-4 py-6 space-y-4">
                ...
                <div id="welcomeMessage" class="text-center py-8">
                    ...
                    <h2 class="text-xl font-semibold mb-2" style="color: var(--text-primary);">Hola, <?= htmlspecialchars(explode(' ', $user['name'])[0]) ?> 👋</h2>
                    <p style="color: var(--text-secondary);" class="max-w-sm mx-auto">Estoy aquí para escucharte. Cuéntame, ¿cómo te sientes hoy?</p>
                </div>
    ...
    <div id="sentimentIndicator" class="fixed bottom-32 left-1/2 transform -translate-x-1/2 hidden">
        <div class="rounded-full px-4 py-2 shadow-lg flex items-center gap-3" style="background-color: var(--bg-secondary); border: 1px solid var(--border-color);">
            <span class="text-xs" style="color: var(--text-secondary);">Tu ánimo:</span>
            <div class="flex gap-1">
                <div id="moodDot1" class="w-2 h-2 rounded-full transition-colors" style="background-color: var(--border-color);"></div>
                <div id="moodDot2" class="w-2 h-2 rounded-full transition-colors" style="background-color: var(--border-color);"></div>
                <div id="moodDot3" class="w-2 h-2 rounded-full transition-colors" style="background-color: var(--border-color);"></div>
                <div id="moodDot4" class="w-2 h-2 rounded-full transition-colors" style="background-color: var(--border-color);"></div>
                <div id="moodDot5" class="w-2 h-2 rounded-full transition-colors" style="background-color: var(--border-color);"></div>
            </div>
        </div>
    </div>
    ...
    <div id="loadingIndicator" class="fixed bottom-32 left-1/2 transform -translate-x-1/2 hidden">
        <div class="rounded-full px-5 py-3 shadow-lg flex items-center gap-3" style="background-color: var(--bg-secondary); border: 1px solid var(--border-color);">
            <div class="flex gap-1">
    ...
    <!-- Input Area -->
    <div class="fixed bottom-0 left-0 right-0" style="background-color: var(--bg-secondary); border-top: 1px solid var(--border-color);">
        <div class="max-w-2xl mx-auto px-4 py-4">
            <div class="rounded-2xl transition-all" style="background-color: var(--bg-input); border: 1px solid var(--border-color);">
            ...
            <p class="text-center text-xs mt-2" style="color: var(--text-tertiary);">Todo lo que compartas es confidencial y seguro 🔒</p>
        </div>
    </div>
    ...
    <div id="logoutModal" class="modal-overlay">
        <div class="modal-content" style="max-width: 24rem;">
            <div class="modal-body text-center py-6">
                ...
                <p style="color: var(--text-secondary);" class="mb-6">Siempre puedes volver cuando necesites hablar. Cuídate mucho. 💜</p>
                <div class="flex gap-3">
                    <button onclick="closeLogoutModal()" class="flex-1 py-2.5 px-4 rounded-xl transition-colors" style="border: 1px solid var(--border-color); color: var(--text-primary); background-color: var(--bg-tertiary);">
                        Cancelar
                    </button>
            ...
            <div class="modal-body" style="overflow-y: auto;">
                ...
                <div id="resource-breathing" class="resource-card">
                    ...
                    <div class="resource-card-content">
                        <h4 style="color: var(--text-primary); font-weight: 600; margin-bottom: 0.5rem;">Respiración 4-7-8 (Calma rápida)</h4>
                        ...
                        <h4 style="color: var(--text-primary); font-weight: 600; margin: 1rem 0 0.5rem 0;">Box Breathing (Respiración cuadrada)</h4>
                        <ol>
                ...
                <div id="resource-grounding" class="resource-card">
                    ...
                    <div class="resource-card-content">
                        ...
                        <p style="font-style: italic; margin-top: 0.75rem;">Tómate tu tiempo con cada paso. No hay prisa.</p>
                    </div>
                </div>
                ...
                <div id="resource-phrases" class="resource-card">
                    ...
                    <div class="resource-card-content">
                        <div id="supportive-phrase" class="supportive-phrase">
                            <p>"Esto que sientes es temporal. Has superado días difíciles antes y lo harás de nuevo."</p>
                        </div>
                        <button class="resource-btn resource-btn-secondary" onclick="showNewPhrase()" style="width: 100%;">
                            Ver otra frase
                        </button>
                    </div>
                </div>
